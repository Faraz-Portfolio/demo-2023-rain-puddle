...
// Hacky, yikes!
function isConstructor<T extends MaterialConstructor>(
  f: T | InstanceType<T>
): f is T {
  try {
    // @ts-ignore
...
> extends THREE.Material {
  ...
  clone() {
    ...
    const clone = new (this.constructor as new (opts: iCSMParams<T>) => this)(
      opts
    );
  ...
  private _generateMaterial(
    ...
    const customOnBeforeCompile = (shader: THREE.Shader) => {
      try {
        ...
        if (fragmentShader) {
          const patchedFragmentShader = this._patchShader(
            stripComments(fragmentShader),
        ...
        if (vertexShader) {
          const patchedVertexShader = this._patchShader(
            stripComments(vertexShader),
            shader.vertexShader
          );
  ...
  private _patchShader(
    ...
    Object.entries(requiredPropsMap).forEach(([prop, keywords]) => {
      const key = keywords.find((keyword) =>
        isExactMatch(customShader, keyword)
      );
      if (key) {
        // @ts-ignore
        if (!this[prop]) {
          throw new Error(
            `CSM: Property "${prop}" is required to use output "${key}". Shader cannot compile.`
          );
        }
      }
    });
      ...
      Object.keys(patchMap[name]).forEach((key) => {
        ...
        if (name === "*" || isExactMatch(customShader, name)) {
          if (
            ...
            throw new Error(
              `CSM: ${name} is not available in ${type}. Shader cannot compile.`
            );
          }
        }
      });
    ...
    const csmMainFunctionName =
      "csm_main_" + this.__csm.instanceID.replace(/-/g, "_");
    const renamedCustomShader = customShader.replace(
      /void\s+main\s*\(\s*\)/g,
      `void ${csmMainFunctionName}()`
    );
